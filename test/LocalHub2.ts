...
import type { LockMap, LockProps } from "@itwin/core-backend";
import { BriefcaseLocalValue, IModelDb, IModelHost, IModelJsFs, LockConflict, LockState, SnapshotDb, SQLiteDb } from "@itwin/core-backend";
...
export class LocalHub2 {
  ...
  public constructor(public readonly rootDir: LocalDirName, arg: LocalHubProps) {
    ...
    if (arg.openExisting && fs.existsSync(this.mockDbName)) {
      this._hubDb = new SQLiteDb();
      this._hubDb.openDb(this.mockDbName, OpenMode.ReadWrite);

      this._hubDb.withSqliteStatement("SELECT max(csIndex) from timeline", (stmt) => {
        if (stmt.step() === DbResult.BE_SQLITE_ROW)
          this._latestChangesetIndex = stmt.getValueInteger(0);
      });
