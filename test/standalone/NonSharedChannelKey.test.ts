...
describe("iTwin Connector Fwk (#standalone) - Using non-shared Channel Key", () => {
  ...
  const nonSharedChanKeyTestConnector = path.join("..", "lib", "test", "TestConnector", "NonSharedChannelKeyTestConnector.js");
  ...
  async function runConnector() {
    ...
    const status = await runner.run(nonSharedChanKeyTestConnector);
  ...
  it("test connector update", async () => {
    ...
    const runner = new ConnectorRunner(jobArgs, hubArgs);
    await runner.run(nonSharedChanKeyTestConnector);
