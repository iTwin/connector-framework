...
describe("iTwin Connector Fwk (#standalone) - Multifile Test", () => {
  ...
  beforeEach(async () => {
    ...
    const assetPath = path.join(KnownTestLocations.assetsDir, "RegPolys.json");
    jobArgs = new JobArgs({
      source: assetPath,
  ...
  async function runConnector() {
    ...
    try {
      utils.verifyRegPolyModel(db, jobArgs);
    } finally {
      db.close();
    }
  }
