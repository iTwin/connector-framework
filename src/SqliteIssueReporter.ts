...
export class SqliteIssueReporter implements ConnectorIssueReporter {
  ...
  private _sourceFileTempConnectorIssuesTable = "BadgersSrcFileReports";
  ...
  public recordReferenceFileInfo(sourceId: string, name: string, uniqueName: string, itemType: string, dataSource: string, downloadUrl: string, state: string, failureReason: string, exists: boolean, fileSize: number, foundByConnector: boolean) {
    const values = [this._contextInfo.activityId, this._contextInfo.contextId, this._contextInfo.jobId, this._contextInfo.iModelId, this._contextInfo.briefcaseId, sourceId, this._fileInfo.filePath,
      "ReferenceFile", itemType, dataSource, name, exists, downloadUrl, state, failureReason, uniqueName, fileSize, foundByConnector];
    this._reportDb.withPreparedSqliteStatement(`INSERT INTO ${this._sourceFileTempConnectorIssuesTable}(activityId, contextId, jobId, imodelId, briefcaseId, fileId, filePath, dataType, itemType, dataSource, fileName, itemExists, downloadUrl, state, failureReason, uniqueName, fileSize, foundByConnector) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`, (stmt) => {
      stmt.bindValues(values);
      stmt.step();
    });
  ...
  public createJsonReport() {
    ...
    this._reportDb.withPreparedSqliteStatement(`SELECT dataType, itemType, dataSource, fileId, fileName, itemExists, downloadUrl, state, failureReason, uniqueName, fileSize FROM ${this._sourceFileTempConnectorIssuesTable} WHERE activityId=? AND foundByConnector=? AND dataType=?`,
      (stmt) => {
    ...
    this._reportDb.withPreparedSqliteStatement(`SELECT dataType, itemType, dataSource, fileId, fileName, itemExists, downloadUrl, state, failureReason, uniqueName, fileSize  FROM ${this._sourceFileTempConnectorIssuesTable} WHERE activityId =? AND dataType=?`, (stmt) => {
      stmt.bindValue(1, this._contextInfo.activityId);
