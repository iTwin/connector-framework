...
*--------------------------------------------------------------------------------------------*/
import { BackendLoggerCategory, DefinitionElement, DefinitionModel, DefinitionPartition, IModelDb, Model, Subject } from "@itwin/core-backend";
import type { Id64Array, Id64String} from "@itwin/core-bentley";
import { LogLevel } from "@itwin/core-bentley";
import { assert as beAssert, DbResult, Logger } from "@itwin/core-bentley";
import {
...
class ElementTreeDeleter extends ElementTreeBottomUp {
  ...
  public deleteElementTree(scope?: ElementTreeWalkerScope): void {
    ...
    // Now the (unused) definitions can be deleted. Delete them in bottom-up order!
    this._definitions.forEach((e) => {
      if (Logger.isEnabled(loggerCategory, LogLevel.Trace)) {
        const el = this._imodel.elements.getElement<DefinitionElement>(e);
        Logger.logTrace(loggerCategory, `Delete definition element ${el.id} ${el.classFullName} ${el.code.value} ${el.getDisplayLabel()}`);
      }
      this._imodel.elements.deleteDefinitionElements([e]); // (will not delete definitions that are still in use)
    });

    for (const m of this._definitionModels) {
      try {
        if (Logger.isEnabled(loggerCategory, LogLevel.Trace)) {
          const model = this._imodel.models.getModel(m);
          Logger.logTrace(loggerCategory, `Delete definition model ${model.id} ${model.classFullName} ${model.name}`);
        }
    ...
    for (const e of this._subjects) {
      try {
        if (Logger.isEnabled(loggerCategory, LogLevel.Trace)) {
          const el = this._imodel.elements.getElement<Subject>(e);
          Logger.logTrace(loggerCategory, `Delete Subject ${el.id} ${el.classFullName} ${el.code.value} ${el.description} ${el.getDisplayLabel()}`);
        }
...
abstract class ElementTreeTopDown {
  ...
  private processSubModel(subModel: Model, scope: ElementTreeWalkerScope) {
    ...
    // This is a top-down traversal, so we visit only the top-level parents. Then, processElementTree will recurse into their children.
